...
public static partial class IsExtensions
{
    ...
    {
        return AssertionConditionCombiner.Combine(@is.AssertionBuilder, new DelegateAssertCondition<TimeOnly, TimeOnly, TAnd, TOr>(@is.AssertionBuilder.AppendCallerMethod(doNotPopulateThisValue), default, (value, _, _, _) =>
            {
    ...
    public static BaseAssertCondition<TimeOnly, TAnd, TOr> IsBetween<TAnd, TOr>(this IIs<TimeOnly, TAnd, TOr> @is, TimeOnly lowerBound, TimeOnly upperBound, [CallerArgumentExpression("lowerBound")] string doNotPopulateThisValue1 = "", [CallerArgumentExpression("upperBound")] string doNotPopulateThisValue2 = "")
        ...
        where TOr : Or<TimeOnly, TAnd, TOr>, IOr<TimeOnly, TAnd, TOr>
    {
        return AssertionConditionCombiner.Combine(@is.AssertionBuilder, new DelegateAssertCondition<TimeOnly, TimeOnly, TAnd, TOr>(@is.AssertionBuilder.AppendCallerMethodWithMultipleExpressions([doNotPopulateThisValue1, doNotPopulateThisValue2]), default, (value, _, _, _) => value >= lowerBound && value <= upperBound,
